...
- **Sector Exposure Simulator & Heatmap** (embedded in External Portfolio tab): GICS-aligned sector/industry classifications (`research_sectors.json`), consolidated weight calculations merging trading account holdings (Alpaca paper API) and external broker records (`EquityLot`), comparison vs. S&P 500 GICS baseline (`sp500_sector_weights.json`), active tilt heatmap visualization, and automated allocation drift alerts exceeding 5 percentage points.
