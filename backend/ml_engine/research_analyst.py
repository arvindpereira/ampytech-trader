...
from ml_engine.analyst_synthesizer import synthesize_theme, synthesize_ticker
...
def build_report(
    ...
    if routed.intent == "ticker_outlook":
        ...
        syn = synthesize_ticker(ticker, facts, items, tier=route.tier, query=routed.raw_query)
        if syn.get("error"):
            syn = {"caveats": syn.get("caveats", [])}
        progress(85, "Assembling report template")
...
def prepare_context(tickers: List[str], db) -> tuple:
    ...
    return facts_by_ticker, items_by_ticker, coverage
